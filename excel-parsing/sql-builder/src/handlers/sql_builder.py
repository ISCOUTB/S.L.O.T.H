...
def sql_builder_handler(
    ...
    columns = {
        col: utils.parse_output_to_proto(col_data)
        for col, col_data in request.columns.items()
    }
    dtypes_map = dict(request.dtypes)
    ...
    output = sql_builder.sql_builder(cols=columns, dtypes=dtypes_map, table_name=table_name)
